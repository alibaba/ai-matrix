...
def main(args):
    ...
    directory_structure = {
        ...
        'extracted' : data_dir +'/extracted',    # Extracted from whatever the initial format is (e.g., wikiextractor)
        'formatted' : data_dir + '/formatted_one_article_per_line',    # This is the level where all sources should look the same
        'sharded' : data_dir + '/sharded_' + "training_shards_" + str(args.n_training_shards) + "_test_shards_" + str(args.n_test_shards) + "_fraction_" + str(args.fraction_test_set),
        'tfrecord' : data_dir + '/tfrecord'+ hdf5_tfrecord_folder_prefix,
        'hdf5': data_dir + '/hdf5' + hdf5_tfrecord_folder_prefix
    }
    ...
    if args.action == 'download':
        ...
        if args.dataset == 'bookscorpus':
            ...
            if args.skip_wikiextractor == 0:
                path_to_wikiextractor_in_container = data_dir + '/tools/wikiextractor/WikiExtractor.py'
        ...
        def create_record_worker(filename_prefix, shard_id, output_format='tfrecord'):
            bert_preprocessing_command = 'python ' + work_dir + '/create_pretraining_data.py'
