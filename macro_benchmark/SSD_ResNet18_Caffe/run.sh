...
do
    ...
    echo "----------------------------------------------------------------"
    start=`date +%s%N`
    python examples/ssd/score_ssd_pascal_resnet18_321.py --batch_size=$batch --num_accelerators=$NUM_ACCELERATORS |& tee ../SSD_ResNet18_Caffe/results/result_infer_${batch}.txt
    end=`date +%s%N`
    total_time=$(((end-start)/1000000))
    #total_time=`bc <<< "scale = 3; ($end-$start)/1000000000"`
    #total_images=$(($batch*$MAX_ITER*$NUM_ACCELERATORS))
    ...
    system_performance=$((1000*$total_images/$total_time))
    echo "Total images is: $total_images" >> ../SSD_ResNet18_Caffe/results/result_infer_${batch}.txt
    echo "Total running time in miliseconds is: $total_time" >> ../SSD_ResNet18_Caffe/results/result_infer_${batch}.txt
    echo "Approximate accelerator performance in images/second is: $system_performance" >> ../SSD_ResNet18_Caffe/results/result_infer_${batch}.txt
    echo "System performance in images/second is: $system_performance" >> ../SSD_ResNet18_Caffe/results/result_infer_${batch}.txt
done
