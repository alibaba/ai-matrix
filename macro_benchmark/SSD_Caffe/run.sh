...
for batch in $batchs
do
    rm -r models/VGGNet/VOC0712
    echo "----------------------------------------------------------------"
    echo "Running training with batch size of $batch"
    echo "----------------------------------------------------------------"
    start=`date +%s%N`
    python examples/ssd/ssd_pascal.py --batch_size=$batch --num_accelerators=$NUM_ACCELERATORS --max_iter=$MAX_ITER |& tee ./results/result_train_${batch}.txt
    end=`date +%s%N`
...
do
    ...
    echo "----------------------------------------------------------------"
    start=`date +%s%N`
    python examples/ssd/score_ssd_pascal.py --batch_size=$batch --num_accelerators=$NUM_ACCELERATORS |& tee ./results/result_infer_${batch}.txt
    end=`date +%s%N`
    total_time=$(((end-start)/1000000))
    #total_time=`bc <<< "scale = 3; ($end-$start)/1000000000"`
    #total_images=$(($batch*$MAX_ITER*$NUM_ACCELERATORS))
