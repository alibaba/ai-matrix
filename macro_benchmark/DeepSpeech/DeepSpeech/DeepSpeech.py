...
def create_flags():
    ...
    tf.app.flags.DEFINE_integer ('steps_p_epoch',    0,           'number of steps to run in each epoch, to shorten the test')
...
def train(server=None):
    ...
    def update_progressbar(set_name):
        ...
        if (update_progressbar.current_set_name != set_name or
            ...
            # recreate pbar
            update_progressbar.pbar = progressbar.ProgressBar(maxval=update_progressbar.total_jobs).start()
        ...
        with tf.train.MonitoredTrainingSession(master='' if server is None else server.target,
            ...
            try:
                ...
                while job and not session.should_stop():
                    ...
                    if completed_batches > FLAGS.steps_p_epoch:
