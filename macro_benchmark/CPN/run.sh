...
do
    ...
    echo "----------------------------------------------------------------"
    start=`date +%s%N`
    python models/COCO.res101.384x288.CPN/network.py -d $CUDA_VISIBLE_DEVICES --batch_size $batch --epoch_size $EPOCH_SIZE --num_epochs $NUM_EPOCHS |& tee ./results/result_train_${batch}.txt
    end=`date +%s%N`
