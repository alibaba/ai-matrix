...
setup.sh installs all the software packages required by the benchmark.
prepare_dataset.sh downloads the coco dataset needed in both training and inference and processes the dataset if necessary.
...
setup.sh and prepare_dataset.sh only need to be ran once, and once the required packages and dataset are set up there is no need to run these two scripts a second time.
