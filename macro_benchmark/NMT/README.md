...
- [Run the Benchmark](#run-the-benchmark)
   ...
   - [Run the Benchmark](#run-benchmark)
   - [Results](#results)
...
## Run the Benchmark
Automation scripts are provided to easily run the benchmark in just a few scripts. There are two sripts for running the benchmark: prepare_dataset.sh and run.sh.

prepare_dataset.sh downloads the dataset needed in both training and inference and processes the dataset if necessary.
...
prepare_dataset.sh only needs to be ran once, and once the dataset is set up there is no need to run it a second time.
