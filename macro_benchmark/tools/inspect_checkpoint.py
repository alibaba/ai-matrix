...
def print_tensors_in_checkpoint_file(file_name, tensor_name, all_tensors,
  ...
  try:
    ...
    if all_tensors or all_tensor_names:
      ...
      # cal the total parameter size
      total_parameter_size = 0
      for key in sorted(var_to_shape_map):
        print("tensor_name: ", key)
        if all_tensors:
          print(reader.get_tensor(key), ',total parameter size=',reader.get_tensor(key).size)
          total_parameter_size += reader.get_tensor(key).size
      if all_tensors:
          print ('Total parameter size=', total_parameter_size)
