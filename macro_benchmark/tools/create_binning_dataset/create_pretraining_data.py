...
def write_instance_to_example_files(instances, tokenizer, max_seq_length,
    ...
    for i in range(8):
        ...
        temp["input_mask"] = []
        temp["segment_ids"] = []
        temp["masked_lm_positions"] =  []
        temp["masked_lm_ids"] = []
        temp["masked_lm_weights"] = []
        temp["next_sentence_labels"] = []
        features.append(temp)
        
        file = output_files[0] + "/length_{0:03d}.hdf5".format((i+1)*16)
    ...
    for (inst_index, instance) in enumerate(instances):
        ...
        if len(input_ids) <= 16:
            writer_index = 0
            seq_length = 16
        elif len(input_ids) <= 32:
            writer_index = 1
            seq_length = 32
        elif len(input_ids) <= 48:
            writer_index = 2
            seq_length = 48
        elif len(input_ids) <= 64:
            writer_index = 3
            seq_length = 64
        elif len(input_ids) <= 80:
            writer_index = 4
            seq_length = 80
        elif len(input_ids) <= 96:
            writer_index = 5
            seq_length = 96
        elif len(input_ids) <= 112:
            writer_index = 6
            seq_length = 112
        else:
            writer_index = 7
    ...
    for i in range(8):
        ...
        features[i]["masked_lm_positions"] = np.array(features[i]["masked_lm_positions"], dtype="int32")
        features[i]["masked_lm_ids"] = np.array(features[i]["masked_lm_ids"], dtype="int32")
        features[i]["masked_lm_weights"] = np.array(features[i]["masked_lm_weights"], dtype="int32")
    ...
    for i in range(8):
        ...
        f.create_dataset("segment_ids", data=features[i]["segment_ids"], dtype='i1', compression='gzip')
        f.create_dataset("masked_lm_positions", data=features[i]["masked_lm_positions"], dtype='i4', compression='gzip')
        f.create_dataset("masked_lm_ids", data=features[i]["masked_lm_ids"], dtype='i4', compression='gzip')
        f.create_dataset("masked_lm_weights", data=features[i]["masked_lm_weights"], dtype='i1', compression='gzip')
        f.create_dataset("next_sentence_labels", data=features[i]["next_sentence_labels"], dtype='i1', compression='gzip')
        f.flush()
        f.close()
