...
__global__ void nms_reduce_batched(const int *n_boxes_arr, unsigned long long *dev_mask_cat, 
  ...
  for (int i = tid; i < col_blocks; i += blockDim.x) res_mask_sh[i] = 0;
  __syncthreads();
...
at::Tensor nms_batched_cuda (const at::Tensor boxes_cat, const std::vector<int> n_boxes_vec, const at::Tensor n_boxes, float nms_overlap_thresh  ){

  //this function assumes input boxes are sorted
  //TODO: add an option for non sorted input boxes
  using scalar_t = float;
  AT_ASSERTM(boxes_cat.type().is_cuda(), "boxes_cat must be a CUDA tensor");
  AT_ASSERTM(n_boxes.type().is_cuda(), "n_boxes must be a CUDA tensor");
  ...
  nms_kernel_batched<<<blocks, threads, 0, stream.stream()>>>(n_boxes.data<int>(),
                                          nms_overlap_thresh,
                                          boxes_cat.data<scalar_t>(),
                                          mask_dev,
                                          n_boxes_max * col_blocks_max);   
                                          
  //last argument is the f_map stride in mask_dev array                                                                               
  nms_reduce_batched<<<n_fmaps, threadsPerBlock, max_shmem_size, stream.stream()>>>(n_boxes.data<int>(), 
                                                                                    mask_dev, 
                                                                                    keep.data<unsigned char>(), 
                                                                                    n_boxes_max * col_blocks_max, 
                                                                                    col_blocks_max); 
