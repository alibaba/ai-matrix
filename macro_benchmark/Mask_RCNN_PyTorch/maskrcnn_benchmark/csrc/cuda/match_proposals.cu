...
at::Tensor match_proposals_cuda(at::Tensor match_quality_matrix, bool allow_low_quality_matches, 
    ...
    at::Tensor best_pred_per_gt = torch::zeros({gt}, at::CUDA(at::kFloat));
    at::Tensor pred_forgiven = torch::zeros({preds}, at::CUDA(at::kByte));    
    at::Tensor intergt = torch::zeros({gt * num_chunks}, at::CUDA(at::kFloat));
