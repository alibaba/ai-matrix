...
class RPNPostProcessor(torch.nn.Module):
    ...
    def forward(self, anchors, objectness, box_regression, targets=None):
        ...
        sampled_boxes = []
        use_fast_cuda_path = (objectness[0].is_cuda and box_regression[0].is_cuda)
        
        ## if running on GPU, separate nms to run it in batched mode for perf
        ## this has an upper bound on pre_nms_top_n, related to GPU shared memory size
        use_batched_nms = use_fast_cuda_path and self.pre_nms_top_n <= 49152 * 4
  
        for a, o, b in zip(anchors, objectness, box_regression):
            sampled_boxes.append(self.forward_for_single_feature_map(a, o, b, nms = (not use_batched_nms)))        
        if use_batched_nms:
            sampled_boxes = self.batched_nms(sampled_boxes)
