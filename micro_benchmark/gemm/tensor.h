...
template <typename T>
Tensor<T> zeros(std::vector<int> dims, T flag) {
    Tensor<T> tensor(dims);
    T val = 0.;
    thrust::fill(thrust::device_ptr<T>(tensor.begin()),
                 thrust::device_ptr<T>(tensor.end()), val);
...
Tensor<float> rand_float(std::vector<int> dims, curandGenerator_t curand_gen) {
    Tensor<float> tensor(dims);
    curandGenerateUniform(curand_gen, tensor.begin(), tensor.size());
    return tensor;
}
Tensor<double> rand_double(std::vector<int> dims, curandGenerator_t curand_gen) {
